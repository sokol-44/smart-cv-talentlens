...
import { Projekt, LocalBookmarks, LocalNotes } from "../types";
import { FolderGit2, Search, ExternalLink, Calendar, Bookmark, MessageSquare, Save, Settings, Layers, Code, Award, Edit3, X, Plus } from "lucide-react";
...
interface ProjektyListProps {
  ...
  onEditProject: (project: Projekt) => void;
  lang: "pl" | "en";
 ...
 * bookmarks management, recruiter note-taking, and modal edit access.
 *
...
export const ProjektyList: React.FC<ProjektyListProps> = ({
  ...
  const allProjectTechs = useMemo(() => {
    ...
    projects.forEach((p) => p.technologie.forEach((t) => techs.add(t)));
  ...
  const filteredProjects = useMemo(() => {
    return projects.filter((p) => {
      const translatedName = p.nazwa;
      const translatedOpis = translate(p.opis, lang);
      const translatedFirms = p.firma.join(", ");
      ...
      const matchesTech =
        selectedTech === "all" ||
        p.technologie.some((t) => t.toLowerCase() === selectedTech.toLowerCase());
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredProjects.map((p) => {
          ...
          return (
            <div
              key={p.id}
              className="bg-white rounded-2xl border border-slate-100 shadow-xs hover:shadow-md transition flex flex-col justify-between overflow-hidden"
            >
              <div className="p-5 md:p-6">
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    ...
                    <div className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide mt-0.5">
                      {translate("Firma:", lang)} {p.firma.join(", ")}
                    </div>
                  </div>
                ...
                <div className="mb-4 inline-flex items-center gap-1 text-[11px] font-mono text-slate-500">
                  ...
                  <span>
                    {p.data.start} – {p.data.end || translate("obecnie", lang)}
                  </span>
                </div>
                  ...
                  <div className="mb-4 p-3 bg-emerald-50 border border-emerald-200 rounded-xl space-y-3">
                    <div>
                      <label className="block text-xs font-bold text-emerald-800 uppercase mb-1">
                        {translate("Dodaj komentarz / uwagę do tego projektu:", lang)} (PL)
                      </label>
                      <textarea
                        value={noteTextPl}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-emerald-800 uppercase mb-1">
                        {translate("Dodaj komentarz / uwagę do tego projektu:", lang)} (EN)
                      </label>
                      <textarea
                        value={noteTextEn}
                ...
                <div className="text-xs text-slate-600 leading-relaxed mb-4">
                  <SupplementaryText text={translate(p.opis, lang)} />
                </div>

                {/* Wyrozniaszajace elementy */}
                {p.wyrozniajajace_sie_elementy && p.wyrozniajajace_sie_elementy.length > 0 && (
                  <div className="mb-4 bg-slate-50/70 p-3 rounded-xl border border-slate-100">
                    ...
                    <ul className="space-y-2 text-[11px] text-slate-600 leading-relaxed">
                      {p.wyrozniajajace_sie_elementy.map((item, idx) => {
